<template>
  <div class="space-y-5">
    <div class="flex items-end justify-between border-b border-edge pb-3">
      <div>
        <p class="font-display font-bold text-sm tracking-wide text-faint uppercase mb-1">Log</p>
        <h1 class="font-display text-lg font-bold">History</h1>
      </div>
      <p class="text-xs text-faint">{{ topics.length }} topik tercatat</p>
    </div>

    <div v-if="loading" class="flex items-center gap-3 text-soft py-8">
      <span class="spinner"></span>
      <span class="text-sm">Memuat...</span>
    </div>

    <div v-else-if="!topics.length" class="border border-edge rounded px-6 py-12 text-center">
      <p class="text-soft text-sm">Belum ada history. Mulai generate di halaman Generate.</p>
    </div>

    <div v-else class="border border-edge rounded overflow-hidden">
      <div
      ...
      >
        <button
          class="w-full px-4 sm:px-5 py-3 flex items-center justify-between hover:bg-surface-hover transition-colors text-left"
          @click="toggleTopic(topic.id)"
        >
          <div class="min-w-0 pr-4">
            <h3 class="font-display font-semibold text-sm truncate">{{ topic.name }}</h3>
          ...
          <div class="flex items-center gap-3 flex-shrink-0">
            ...
            <button
              class="text-danger text-xs font-semibold px-2.5 py-1 rounded hover:bg-danger/10 transition-colors"
              @click.stop="handleDelete(topic.id)"
            >Hapus</button>
          </div>
        </button>

        <div v-if="expanded === topic.id" class="border-t border-edge px-4 sm:px-5 py-4 space-y-3 bg-surface-strong/40">
          <div v-if="detailLoading === topic.id" class="flex items-center gap-2 text-soft py-4">
            <span class="spinner-sm"></span>
            <span class="text-sm">Memuat...</span>
          </div>
          <template v-else-if="detail">
            <div v-for="idea in detail.ideas" :key="idea.id" class="bg-surface rounded p-4">
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2 flex-wrap">
                    <span
                      v-if="idea.angle_category"
                      class="text-[10px] px-1.5 py-0.5 rounded bg-accent/10 text-accent border border-accent/20 font-semibold slate-tag uppercase"
                    >{{ idea.angle_category }}</span>
                    <span
                      v-if="idea.total_score"
                      class="text-[10px] px-1.5 py-0.5 rounded bg-surface-strong text-soft border border-edge slate-tag"
                ...
                <span
                  class="text-[11px] px-2 py-0.5 rounded font-semibold flex-shrink-0 slate-tag uppercase"
              ...
              <div v-if="idea.prompts?.length" class="space-y-2 mt-3">
                <div v-for="p in idea.prompts" :key="p.id" class="glass-strong rounded p-3.5 text-sm">
                  <div class="flex items-center gap-2 flex-wrap">
              ...
              <button
                v-else
                class="mt-3 text-xs font-semibold bg-accent/10 hover:bg-accent/20 text-accent border border-accent/25 px-3.5 py-1.5 rounded transition-colors flex items-center gap-2"
  ...
  <div v-if="showDeleteConfirm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" @click.self="showDeleteConfirm = null">
    <div class="glass-strong p-6 max-w-sm w-full">
      <div class="flex items-center gap-3 mb-4">
        <span class="w-10 h-10 rounded bg-danger/12 text-danger flex items-center justify-center flex-shrink-0">
      ...
      <div class="flex gap-3 justify-end">
        <button class="text-sm font-semibold text-soft hover:text-text px-4 py-2 rounded glass glass-hover transition-colors" @click="showDeleteConfirm = null">Batal</button>
        <button class="text-sm font-semibold bg-danger hover:opacity-90 px-4 py-2 rounded text-white transition-colors" @click="confirmDelete">Hapus</button>
      </div>
    </div>
  </div>
</template>
